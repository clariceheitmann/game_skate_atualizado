...
let coracao = new Coracao(0, 0, 90, 90, './imgs/coracao.png')
...
function spawnCoracao(){
    if(!coracao.ativo){
        ...
        if(chance < 0.002){ // 🔥 controla raridade
            coracao.spawn()
        }
    }
}
...
function ver_fase() { 
    ...
    fase = 4
    maxInimigos = 3
    inimigos.forEach(i => i.vel = 14)
    fundo = fundo4
}
else if (pontosRef > 110 && fase === 4) {
    fase = 5
    maxInimigos = 4
    inimigos.forEach(i => i.vel = 16)
    fundo = fundo5
}
}
...
function desenhaMenu(){
    ...
    botaoComoJogar = { x: 600 - larguraTexto / 2, y: 400 - 20, w: larguraTexto, h: 30 }
